...
- All hosts are shared. `nvidia-smi` + check process owner before launching. `./gpu.sh` = live all-6 dashboard, 5s refresh, Ctrl+C to quit (`--once` snapshot, `--selftest` parser checks). `free*` means "nothing allocated right now", not "unowned".
